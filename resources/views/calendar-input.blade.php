...
>
    <div wire:ignore class="w-full">
        ...
        formatStateDate(dateString) {
            ...
            return isNaN(date) ? dateString : date.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        },
        ...
        getCurrentMonthYearFormatted() {
            return this.initialMonthYear || this.currentDate.toLocaleDateString('es-ES', { year: 'numeric', month: 'long' });
        },

        getMonthYearForDate(date) {
            // Usar el formato inicial de PHP si la fecha coincide con el mes actual
            if (date.getMonth() === new Date().getMonth() && date.getFullYear() === new Date().getFullYear()) {
                return this.getCurrentMonthYearFormatted();
            }
                ...
                <div class="relative flex justify-center gap-4 p-4">
                    <div>
                        ...
                        <div class="flex items-center justify-between h-10 px-2 mb-4 sm:h-8">
                            ...
                            <div class="text-sm font-medium text-zinc-800 dark:text-white" x-text="getMonthYearForDate(currentDate)"></div>
                            ...
                            <thead>
                                <tr class="flex w-full">
                                    <template x-for="day in daysOfWeek">
                                        <th scope="col" class="flex items-center text-sm font-medium size-11 sm:size-11 text-zinc-500 dark:text-zinc-300">
                                ...
                                <template x-for="week in calendarWeeks" :key="week.key">
                                    <tr class="flex w-full not-first-of-type:mt-1">
                                        <template x-for="date in week.days" :key="date.key">
                                            <td
                                                class="p-0 data-in-range:bg-zinc-100 dark:data-in-range:bg-white/10 first-of-type:rounded-s-lg last-of-type:rounded-e-lg"
                                                :class="{
                                            ...
                                            >
                                                <button
                                                    ...
                                                    :class="{
                                                        'bg-primary-500 text-white hover:bg-primary-600': date.isSelected && !date.isDisabled,
                                                        'hover:bg-zinc-800/5 dark:hover:bg-white/5': !date.isSelected && !date.isDisabled && !isDisabled
                                                    }"
                                                ...
                                                >
                                                    <div class="relative">
                                                        <!-- Today indicator dot -->
                                                        <div
                                                            x-show="date.isToday && !date.isDisabled"
                                                            class="absolute inset-x-0 bottom-[-3px] flex justify-center items-end"
                                                        >
                                                            <div
                                                                class="rounded-full size-1 bg-zinc-800 dark:bg-white"
